...
const Lov = props => {
  ...
  const handleTableChange = (type, { page, sortField, sortOrder, sizePerPage }) => {
    ...
    if (type === "pagination") {
      setReq({ page: page, limit: sizePerPage, offset: ((page - 1) * sizePerPage), sort: props.stateSearchInput.sort, order: props.stateSearchInput.order, search: { any: searchValModal, param: props.pParam } });
    }
  };
